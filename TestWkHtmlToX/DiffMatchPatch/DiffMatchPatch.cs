...
namespace DiffMatchPatch
{
    ...
    internal static class CompatibilityExtensions
    {
        ...
        // public delegate bool selector_t<TEnumerableElement>(TEnumerableElement arg);


        public static TEnumerableElement First<TEnumerableElement>(IEnumerable<TEnumerableElement> enumerable)
        {
            if (enumerable == null) throw new System.ArgumentNullException("enumerable");

            IList<TEnumerableElement> listInstance = enumerable as IList<TEnumerableElement>;
            if (listInstance != null)
            {
                if (listInstance.Count > 0) return listInstance[0];
            }
            else
            {
                using (IEnumerator<TEnumerableElement> enumerator = enumerable.GetEnumerator())
                {
                    if (enumerator.MoveNext()) return enumerator.Current;
                }
            }

            throw new System.InvalidOperationException("The sequence does not contain any elements.");
        }


        //public static TEnumerableElement First<TEnumerableElement>(IEnumerable<TEnumerableElement> enumerable, selector_t<TEnumerableElement> selector)
        //{
        //    if (enumerable == null) throw new System.ArgumentNullException("enumerable");
        //    if (selector == null) throw new System.ArgumentNullException("selector");

        //    foreach (TEnumerableElement thisElement in enumerable)
        //    {
        //        if (selector(thisElement)) return thisElement;
        //    }

        //    throw new System.InvalidOperationException("The sequence does not contain a matching element.");
        //}



        public static TEnumerableElement Last<TEnumerableElement>(IEnumerable<TEnumerableElement> enumerable)
        {
            if (enumerable == null) throw new System.ArgumentNullException("enumerable");
            ...
            if (listInstance != null)
            {
                int listElementCount = listInstance.Count;
                if (listElementCount > 0) return listInstance[listElementCount - 1];
            }
            else
            {
                using (IEnumerator<TEnumerableElement> enumerator = enumerable.GetEnumerator())
                {
                    ...
                    {
                        TEnumerableElement lastElement;
                        do
                        {
        ...
        //public static TEnumerableElement Last<TEnumerableElement>(IEnumerable<TEnumerableElement> enumerable, selector_t<TEnumerableElement> selector)
        //{
        //    // if (enumerable == null) throw ErrorFactory.ArgumentNull("enumerable");
        //    // if (predicate == null) throw ErrorFactory.ArgumentNull("predicate");
        //    if (enumerable == null) throw new System.ArgumentNullException("enumerable");
        //    if (selector == null) throw new System.ArgumentNullException("selector");

        //    bool foundMatch = false;
        //    TEnumerableElement lastMatch = default(TEnumerableElement);

        //    foreach (TEnumerableElement thisElement in enumerable)
        //    {
        //        if (selector(thisElement))
        //        {
        //            lastMatch = thisElement;
        //            foundMatch = true;
        //        }
        //    }
        //    if (foundMatch) return lastMatch;

        //    // throw ErrorFactory.NoMatch();
        //    throw new System.InvalidOperationException("The sequence does not contain a matching element.");
        //}
        ...
        public static List<T> Splice<T>(List<T> input, int start, int count,
            params T[] objects)
        {
    ...
    public class diff_match_patch
    {
                                              ...
                                              Dictionary<string, int> lineHash)
        {
            ...
            while (lineEnd < text.Length - 1)
            {
                ...
                line = CompatibilityExtensions.JavaSubstring(text, lineStart, lineEnd + 1);
                lineStart = lineEnd + 1;
                ...
                switch (diffs[pointer].operation)
                {
                    ...
                    case Operation.EQUAL:
                        ...
                        if (count_delete + count_insert > 1)
                        {
                            ...
                            if (count_delete == 0)
                            {
                                CompatibilityExtensions.Splice(diffs, pointer - count_insert,
                                    count_delete + count_insert,
                                    new Diff(Operation.INSERT, text_insert));
                            }
                            else if (count_insert == 0)
                            {
                                CompatibilityExtensions.Splice(diffs, pointer - count_delete,
                                    count_delete + count_insert,
                            ...
                            {
                                CompatibilityExtensions.Splice(diffs, pointer - count_delete - count_insert,
                                    count_delete + count_insert,
                                    new Diff(Operation.DELETE, text_delete),
                                    new Diff(Operation.INSERT, text_insert)
                                );
                  ...
                  diffs[pointer + 1].operation == Operation.EQUAL)
                {
                        ...
                        StringComparison.Ordinal))
                    {
                        ...
                        diffs[pointer + 1].text = diffs[pointer - 1].text
                            + diffs[pointer + 1].text;
                        CompatibilityExtensions.Splice(diffs, pointer - 1, 1);
                        changes = true;
                    }
        ...
        public string diff_prettyHtml(List<Diff> diffs)
        {
            ...
            foreach (Diff thisDiff in diffs)
            {
                //string text = thisDiff.text.Replace("&", "&amp;")
                //    .Replace("<", "&lt;").Replace(">", "&gt;")
                //    .Replace("\r\n", "&para;<br>").Replace("\n", "&para;<br>")
                //    .Replace(" ", "&nbsp");


                string text = System.Web.HttpUtility.HtmlEncode(thisDiff.text)
                    .Replace("\r\n", "&para;<br>").Replace("\n", "&para;<br>")
                    //.Replace(" ", "&nbsp")
                ;
                ...
                {
                    case Operation.INSERT:
                        //html.Append("<ins class=\"insertion\">")
                        html.Append("<ins>")
                            .Append(text)
                            .Append("</ins>");
                        break;
                    case Operation.DELETE:
                        //html.Append("<del class=\"deletion\">")
                        html.Append("<del>")
                            .Append(text)
                            .Append("</del>");
                        break;
        ...
        protected void patch_addContext(Patch patch, string text)
        {
                ...
                && pattern.Length < Match_MaxBits - Patch_Margin - Patch_Margin)
            {
                ...
                pattern = CompatibilityExtensions.JavaSubstring(text
            ...
            string prefix = CompatibilityExtensions.JavaSubstring(text, Math.Max(0, patch.start2 - padding), patch.start2);
            if (prefix.Length != 0)
            {
                patch.diffs.Insert(0, new Diff(Operation.EQUAL, prefix));
            }
            // Add the suffix.
            string suffix = CompatibilityExtensions.JavaSubstring(text
                ...
                switch (aDiff.operation)
                {
                    ...
                    case Operation.EQUAL:
                        if (aDiff.text.Length <= 2 * Patch_Margin
                            && patch.diffs.Count != 0 && aDiff != CompatibilityExtensions.Last(diffs))
                        {
        ...
        public Object[] patch_apply(List<Patch> patches, string text)
        {
            ...
            foreach (Patch aPatch in patches)
            {
                ...
                else
                {
                    ...
                    if (end_loc == -1)
                    {
                        text2 = CompatibilityExtensions.JavaSubstring(text
                            ,start_loc
        ...
        public string patch_addPadding(List<Patch> patches)
        {
            ...
            Patch patch = CompatibilityExtensions.First(patches);
            List<Diff> diffs = patch.diffs;
            if (diffs.Count == 0 || CompatibilityExtensions.First(diffs).operation != Operation.EQUAL)
            {
            ...
            else if (paddingLength > CompatibilityExtensions.First(diffs).text.Length)
            {
                // Grow first equality.
                Diff firstDiff = CompatibilityExtensions.First(diffs);
            ...
            patch = CompatibilityExtensions.Last(patches);
            
            diffs = patch.diffs;
            if (diffs.Count == 0 || CompatibilityExtensions.Last(diffs).operation != Operation.EQUAL)
            {
                // Add nullPadding equality.
            ...
            else if (paddingLength > CompatibilityExtensions.Last(diffs).text.Length)
            {
                // Grow last equality.
                Diff lastDiff = CompatibilityExtensions.Last(diffs);
        ...
        public void patch_splitMax(List<Patch> patches)
        {
            ...
            for (int x = 0; x < patches.Count; x++)
            {
                ...
                CompatibilityExtensions.Splice(patches, x--, 1);
                ...
                while (bigpatch.diffs.Count != 0)
                {
                        ...
                        && patch.length1 < patch_size - this.Patch_Margin)
                    {
                        ...
                        if (diff_type == Operation.INSERT)
                        {
                            ...
                            patch.diffs.Add(CompatibilityExtensions.First(bigpatch.diffs));
                            bigpatch.diffs.RemoveAt(0);
                            empty = false;
                        }
                        else if (diff_type == Operation.DELETE && patch.diffs.Count == 1
                          && CompatibilityExtensions.First(patch.diffs).operation == Operation.EQUAL
                    ...
                    if (!empty)
                    {
                        CompatibilityExtensions.Splice(patches, ++x, 0, patch);
